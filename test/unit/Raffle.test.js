    ...
    : describe("Raffle unit test", function () {
          ...
          describe("check upkeep", function () {
              it("returns false if people have not send any eth", async function () {
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
                  const { upKeekNeeded } = await raffle.callStatic.checkUpkeep([])
                  assert(!upKeekNeeded)
              })
              it("returns false if raffle is not open", async function () {
                  ...
                  assert.equal(upKeekNeeded, false)
              })
              it("returns false if not enough time have passed", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() - 1])
                  await network.provider.send("evm_mine", [])
                  const raffleState = await raffle.getRaffleState()
              ...
              it("returns true if enough time have passed", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
                  const { upKeekNeeded } = await raffle.callStatic.checkUpkeep([])
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("picks a winner, resets de lottery, sends  money", async function () {
